var roleUpgrader = {
    ...
    run: function(creep) {
        ...
        if(creep.memory.upgrading) {
            ...
            // Harvesting state: get energy with priority order
            // 中文: 采集状态：按优先级顺序获取能量
            // Priority 0: Link (controller range 2) > Priority 1: Container > Priority 2: Wait > Priority 3: Source
            
            var controller = creep.room.controller;
            
            if(controller) {
                // Priority 0: Get energy from Links near controller (within 2 range)
                // 优先级0：从控制器两格以内的Link获取能量
                var linksNearController = controller.pos.findInRange(FIND_STRUCTURES, 2, {
                    filter: (structure) => {
                ...
                if(linksNearController.length > 0) {
                    var targetLink = linksNearController[0];
                    creep.say('🔗 link');
                    
                    if(creep.withdraw(targetLink, RESOURCE_ENERGY) == ERR_NOT_IN_RANGE) {
                        creep.moveTo(targetLink, {visualizePathStyle: {stroke: '#00ffff'}});
                    }
                }
                else {
