var roleHarvester1 = {
    ...
    run: function(creep) {
        ...
        if(creep.memory.delivering) {
            ...
            // Priority 0: Transfer to Links near source[1]
            // 优先级0：向source[1]两格以内的Link传输能量
            var sources = creep.room.find(FIND_SOURCES);
            var linksNearSource1 = [];
            
            if(sources.length > 1 && sources[1]) {
                linksNearSource1 = sources[1].pos.findInRange(FIND_STRUCTURES, 2, {
                    filter: (structure) => {
            ...
            if(linksNearSource1.length > 0) {
                ...
                creep.say('🔗 link');
                if(creep.transfer(linksNearSource1[0], RESOURCE_ENERGY) == ERR_NOT_IN_RANGE) {
                    creep.moveTo(linksNearSource1[0], {visualizePathStyle: {stroke: '#00ffff'}});
                }
            }
            // Priority 1: Look for containers with free capacity near source[1]
            // 优先级1：寻找 source[1] 附近有空余容量的容器
            else {
                var containers = [];
                
                if(sources.length > 1 && sources[1]) {
                    containers = sources[1].pos.findInRange(FIND_STRUCTURES, 2, {
                        filter: (structure) => {
                ...
                // 优先级2：存在容器且容器不可用时，改为等待
                    ...
                    // Check if containers exist near source[1] (regardless of capacity)
                    // 检查 source[1] 附近是否存在容器（不考虑容量）
                    var allContainers = [];
                    if(sources.length > 1 && sources[1]) {
                        allContainers = sources[1].pos.findInRange(FIND_STRUCTURES, 2, {
                            filter: (structure) => {
                                return structure.structureType == STRUCTURE_CONTAINER;
                            }
                        });
                    ...
                    if(allContainers.length > 0) {
                        ...
                        // 容器存在但已满，修复容器而不是等待
                        var containersToRepair = sources[1].pos.findInRange(FIND_STRUCTURES, 2, {
                            filter: (structure) => {
                                return structure.structureType == STRUCTURE_CONTAINER &&
                    ...
                    // 优先级3：无容器时，则传输到孵化器或扩展结构
                        ...
                        // If no targets available, repair containers near source[1]
                        // 中文: 如果没有可用目标，修复 source[1] 附近的容器
                        else {
                            var repairContainers = [];
                            
                            if(sources.length > 1 && sources[1]) {
                                repairContainers = sources[1].pos.findInRange(FIND_STRUCTURES, 2, {
                                    filter: (structure) => {
                                        return structure.structureType == STRUCTURE_CONTAINER &&
                                               structure.hits < structure.hitsMax;
                                    }
